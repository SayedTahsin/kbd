...
} from "@shell/composables/useLabeledFormElement";
...
export default defineComponent({
  ...
  computed: {
    ...
    hasLabel(): boolean {
      return this.isCompact
        ? false
        : !!this.label || !!this.labelKey || !!this.$slots.label;
    },

    ariaDescribedBy(): string | undefined {
      const inheritedDescribedBy = this.$attrs["aria-describedby"];
      const internalDescribedBy =
        this.cronHint || this.subLabel ? this.describedById : undefined;
    ...
    cronHint(): string | undefined {
      ...
      if (
        !isPredefined &&
        !isValidCron(this.value as string, {
          alias: true,
          allowBlankDay: true,
  ...
  created() {
    ...
    this.onInput = this.delay
      ? debounce(this.delayInput, this.delay)
      : this.delayInput;
  },
  ...
  >
    <slot name="label">
      <label v-if="hasLabel" :for="inputId">
        ...
        <span v-if="requiredField" class="required" :aria-hidden="true">*</span>
      </label>
    </slot>
    ...
    />
    <div v-if="cronHint || subLabel" class="sub-label" data-testid="sub-label">
      <div
        v-if="cronHint"
        :id="describedById"
        role="alert"
      ...
      <div v-else-if="subLabel" :id="describedById" v-clean-html="subLabel" />
    </div>
  </div>
</template>
